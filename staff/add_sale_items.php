...
/* ===============================
   SAVE SALE ITEMS + LOG STOCK MOVEMENT
================================ */
    ...
    try {
        foreach ($_POST['items'] as $item) {
            ...
            if (preg_match('/^\d+/', $quantityRaw, $matches)) {
                ...
                throw new Exception("Quantity must start with a number.");
            }

            // Lock product row
            $stockStmt = $pdo->prepare("SELECT name, quantity, reorder_level FROM products WHERE id=? FOR UPDATE");
            ...
            // Log stock movement immediately
            ...
            $insertMovement->execute([
                ...
                $quantity,
                'SALE-' . $sale_id,
                'Stock deducted due to sale invoice ' . $sale['invoice_no'],
                $staff_id
            ]);
        }
        ...
        $total_amount = (float)$totalStmt->fetchColumn();

        // Update sale total
        $updateSale = $pdo->prepare("UPDATE sales SET total_amount = ? WHERE id = ?");
        $updateSale->execute([$total_amount, $sale_id]);

        $pdo->commit();
        $success = "Sale items saved successfully!";
...
<div class="page-container">
  <div class="card">
    ...
    <?php if($success): ?>
      <div class="alert" style="background:rgba(0,255,157,0.12);color:#00ff9d;"><?= htmlspecialchars($success) ?></div>
    <?php endif; ?>
...
function addRow(){
  ...
  row.innerHTML = `
    ...
    <td><input type="text" name="items[${rowCount}][quantity]" placeholder="e.g. 16sachets" required></td>
...
function setPrice(selectEl){
  ...
  if(price && priceInput) priceInput.value = parseFloat(price).toFixed(2);
}
</script>
